...
class XMPPHP_XMPP extends XMPPHP_XMLStream
{
     ...
     */
    public function message($to, $body, $thread = null, $type = 'chat', $subject = null, $payload = null)
    {
        if (is_null($type)) {
            $type = 'chat';
        }
        ...
        $out .= "<body>$body</body>";
        if ($thread) $out .="<thread>$thread</thread>";
    ...
    {
        $out = "<iq type='set' id='reg2'><query xmlns='jabber:iq:register'><username>{$name}</username><password>{$password}</password></query></iq>";
